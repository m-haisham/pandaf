...
function createSimpleConnectApp(): ConnectApp {
  return {
    handle(req: IncomingMessage, res: ServerResponse, next?: (err?: unknown) => void) {
      if (req.url === "/handled") {
        res.statusCode = 200;
        res.setHeader("x-custom", "vue-responded");
        res.end("Vue handled this");
      } else {
        next?.();
      }
    },
...
describe("mountConnect via @pandaf/vue re-export", () => {
  let app: Elysia;
  ...
  it("delegates to connect when connect handles the request", async () => {
    ...
    expect(res.headers.get("x-custom")).toBe("vue-responded");
    const text = await res.text();
    expect(text).toContain("Vue handled this");
  });
