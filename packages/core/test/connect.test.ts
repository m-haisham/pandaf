...
import { mountConnect, type ConnectApp } from "../src/connect.js";
...
function createSimpleConnectApp(): ConnectApp {
  return {
    handle(req: IncomingMessage, res: ServerResponse, next?: (err?: unknown) => void) {
      if (req.url === "/handled") {
        res.statusCode = 200;
        res.setHeader("x-custom", "vite-responded");
        res.end("Vite handled this");
      } else {
        next?.();
      }
    },
...
describe("mountConnect — basic delegation", () => {
  let app: Elysia;
  ...
  it("delegates to connect when connect handles the request", async () => {
    ...
    expect(res.headers.get("x-custom")).toBe("vite-responded");
    const text = await res.text();
    expect(text).toContain("Vite handled this");
  });
