...
export const app = new Elysia({ adapter: node() })
  ...
  .use(
    openapi({
      ...
      documentation: {
        info: {
          title: "pandaf PDF Service (React)",
          version: "1.0.0",
          description:
            "A small example consumer of `@pandaf/react` that turns React + " +
        ...
        servers: [
          { url: "http://localhost:8081", description: "Local dev server" },
        ],
        tags: [
          {
            name: "PDF Templates",
            description: "Generate or preview a PDF from a React template.",
          },
          {
            name: "API Reference",
      ...
      scalar: {
        ...
        metaData: {
          title: "pandaf PDF Service (React)",
          description:
            "Generate and preview PDFs from React + Tailwind templates.",
        },
      },
    }),
  )
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const mode =
    process.env.NODE_ENV === "production"
      ? "prod, manifest"
      : "dev, live templates";
  console.log(`🦊 pandaf (React) running (${mode}) on :8081`);
}
